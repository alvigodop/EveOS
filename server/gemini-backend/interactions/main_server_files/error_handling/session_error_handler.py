...
class SessionErrorHandler:
    ...
    async def handle_session_error(self, error: Exception, model_name: str = "") -> None:
        """Handle general session initialization errors."""
        await handle_session_initialization_error(error, str(self.connection_id), self.connection_monitor.safe_send, self.api_error_handler, model_name)
