...
class APIErrorHandler:
    def __init__(self):
        ...
        self.MAX_RETRY_ATTEMPTS = 2  # Reduced to prevent quota exhaustion
        self.PREVIEW_MODEL_MAX_RETRIES = 2  # Reduced to prevent quota exhaustion
        self.DEADLINE_ERROR_MAX_RETRIES = 3  # Reduced for faster failure
        self.QUOTA_WAIT_TIME = 60  # seconds
        
        # Use configurable circuit breaker settings
        self.CIRCUIT_BREAKER_THRESHOLD = 5  # Reduced from 8
    ...
    async def handle_api_error(self, error: Exception, connection_id: str, model_name: str = "") -> Tuple[bool, str]:
        ...
        if self._is_quota_error(error_msg):
            ...
            if time_remaining > 0:
                return False, f"API quota exceeded. Please try again in {int(time_remaining)} seconds."
        ...
        if self._is_preview_model_error(error_msg) or is_preview_model:
            ...
            if self.error_counts[connection_id] <= max_retries:
                ...
                await asyncio.sleep(delay)
                return True, f"Preview model issue ({error_msg}) (attempt {self.error_counts[connection_id]}/{max_retries}). Retrying..."
            else:
        ...
        if self.error_counts[connection_id] <= max_retries:
            ...
            await asyncio.sleep(delay)
            return True, f"API error ({error_msg}) - retrying attempt {self.error_counts[connection_id]}/{max_retries}"
    ...
    def _is_quota_error(self, error_msg: str) -> bool:
        ...
        quota_keywords = [
            ...
            "rate limited", "quota depleted", "exceeded your current quota"
        ]
