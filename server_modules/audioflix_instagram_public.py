...
def _video_payload(media: dict[str, Any], *, source: str) -> dict[str, Any] | None:
    ...
    title = str(caption.get("text") if isinstance(caption, dict) else (caption or "")).strip()
...
def resolve_public(shortcode: str) -> dict[str, Any]:
    ...
    # Use Instagram's current logged-out Polaris flow first. It is the closest
    # equivalent to the public browser flow used by current open-source extractors.
