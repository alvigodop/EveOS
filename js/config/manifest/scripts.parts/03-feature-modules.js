/* EveOS Manifest Scripts Part */
window.EveModuleManifestParts = window.EveModuleManifestParts || {};
window.EveModuleManifestScriptChunks = window.EveModuleManifestScriptChunks || [];

window.EveModuleManifestScriptChunks.push([
    // Feature Modules (Consolidated)
    "js/modules/features/favicon-cache.core.js?v=0.1.2",
    "js/modules/features/favicon-cache.runtime.js?v=0.1.4",
    "js/modules/features/favicon-cache.js?v=0.2.0",
    "js/modules/features/weather.js?v=0.1.0",
    "js/modules/features/timer.js?v=0.1.0",
    "js/modules/features/custom-order.js?v=0.1.0",
    "js/modules/features/true-value-engine.js?v=0.1.0",
    "js/modules/features/social-manager/social-manager-embedded.js?v=0.1.1",
    "js/modules/features/social-manager/social-manager-modal.helpers.js?v=0.1.0",
    "js/modules/features/social-manager/social-manager-modal.js?v=0.1.1",
    "js/modules/features/gemini-ask-bar.insights.js?v=0.1.1",
    "js/modules/features/gemini-ask-bar.js?v=0.3.0"
]);
