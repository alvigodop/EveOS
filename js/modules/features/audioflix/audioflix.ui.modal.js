        ...
        const renderInfoModal = (item, type) => {
            ...
            if (type === 'music') {
                ...
                const keepBtn = (item.upstreamMissing && item.playlistId)
                    ...
                    : '';
                const link = conn ? `<div style="margin-top:4px; font-size:0.72rem; color:#94a3b8;" title="${esc(conn.url || '')}">Source: <code style="color:#8ab4f8;">${esc(conn.url || '(none)')}</code></div>` : '';
            ...
            return `<div class="audioflix-info-modal" data-af-action="close-info"><div class="audioflix-info-card"><div class="audioflix-info-header"><div><span class="audioflix-kicker">${type === 'music' ? 'Track Details' : 'Sound Details'}</span><h3 class="audioflix-info-title">${esc(item.title)}</h3></div><button type="button" class="audioflix-info-close-btn" data-af-action="close-info">${closeSvg}</button></div><div class="audioflix-info-body">${row('Type', type)}${row('Source', src)}${row('Duration', dur)}${item.artist ? row('Artist', item.artist) : ''}${item.volume !== undefined ? row('Volume modifier', item.volume) : ''}${exposeRow}${spotifyTransportRow}${hotkeyRow}${repeaterBlock}${dupSection}${renderGroupAssign(item, type)}${trackEditBlock}<div class="audioflix-info-url-container"><span>Audio URL / Path</span><div class="audioflix-info-url-row"><input type="text" readonly value="${esc(item.url)}" class="audioflix-info-url-input" onclick="this.select()"><button type="button" class="audioflix-info-copy-btn" data-af-action="copy-url" data-af-url="${esc(item.url)}">Copy</button></div></div>${(type === 'music' ? uiLoc.renderSongLocalizations(item) : '') || localSourceRow}${provenanceSection}${offlineRow}${type === 'music' ? uiClass.renderSongSection(item) : ''}${songLocalizeSection}</div><div class="audioflix-info-footer">${internalViewButton(item, type, true)}<button type="button" class="audioflix-info-close-action" data-af-action="close-info">Close</button></div></div></div>`;
        };
