...
(function () {
    ...
    function rerender() {
        ...
        window.EveAudioflixSoundLabUi?.setVisible?.(activeTab === 'soundlab');
    ...
    const close = () => { if (overlay) overlay.hidden = true; window.EveAudioflixSoundLabUi?.setVisible?.(false); window.EveAudioflixAudio?.attachWaveform?.(null); window.EveAudioflixLinks?.clearPendingScope?.(); setButtonExpanded(false); stopHotkeyFeedbackPoll(); pushHotkeysToBridge(); };
