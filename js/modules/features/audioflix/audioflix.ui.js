...
(function () {
    ...
    let addFormOpen = { sound: false, music: false }, portsOpen = false, groupsOpen = false, portedSounds = [], collapsedGroups = {}, activeRepeaters = {};
    ...
    function renderInfoModal(item, type) {
        ...
        const repeaterBlock = type === 'sound' ? `<div class="audioflix-info-row" style="display:flex;flex-direction:column;gap:8px;margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.08);"><span style="font-weight:bold;font-size:0.85rem;color:#8ab4f8;">Sound Repeater</span><div style="display:flex;gap:8px;align-items:center;"><label style="display:flex;flex-direction:column;font-size:0.75rem;flex:1;"><span>Interval (sec)</span><input type="number" step="0.1" min="0.1" value="${rep ? rep.intervalMs / 1000 : 1.0}" id="audioflix-rep-interval" style="background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.15);color:#fff;padding:4px;border-radius:4px;font-size:0.8rem;" ${rep ? 'disabled' : ''}></label><label style="display:flex;flex-direction:column;font-size:0.75rem;flex:1;"><span>Count (0 = inf)</span><input type="number" min="0" value="${rep ? rep.count : 0}" id="audioflix-rep-count" style="background:rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.15);color:#fff;padding:4px;border-radius:4px;font-size:0.8rem;" ${rep ? 'disabled' : ''}></label><button type="button" data-af-action="toggle-repeater" data-af-id="${esc(item.id)}" style="margin-top:14px;background:${rep ? '#ea4335' : '#8ab4f8'};color:#000;font-weight:bold;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:0.8rem;">${rep ? 'Stop' : 'Start'}</button></div></div>` : '';
    ...
    function renderGroupAssign(item) {
        const mine = new Set(groupsOf(item.id)), groups = allGroups(), checklist = groups.length ? groups.map((g) => `<label class="audioflix-group-check"><input type="checkbox" class="audioflix-group-cb" data-af-id="${esc(item.id)}" data-af-group="${esc(g)}" ${mine.has(g) ? 'checked' : ''}><span>${esc(g)}</span></label>`).join('') : `<span class="audioflix-group-empty">No groups yet — create one below.</span>`;
        return `<div class="audioflix-info-groups"><span class="audioflix-info-groups-label">Frontend Groups</span><div class="audioflix-group-checklist">${checklist}</div><form class="audioflix-group-quick" data-af-form="assign-new-group" data-af-id="${esc(item.id)}"><input name="name" placeholder="New group" autocomplete="off" maxlength="40"><button type="submit" data-af-action="submit-form">Add</button></form></div>`;
    }
    ...
    function ensureOverlay() {
        ...
        overlay.addEventListener('input', e => {
            ...
            if (t.classList.contains('audioflix-volume-slider')) {
                const vol = parseFloat(t.value), id = t.dataset.afId;
                t.style.setProperty('--vol', `${vol * 100}%`);
                const lbl = t.nextElementSibling; if (lbl) lbl.textContent = `${Math.round(vol * 100)}%`;
        ...
        overlay.addEventListener('change', async e => {
            ...
            if (t.classList.contains('audioflix-expose-cb')) {
                ...
                window.EveAudioflixState?.toggleSoundGroup?.(id, t.dataset.afGroup, t.checked);
                pushHotkeysToBridge();
                ...
                try {
                    if (ctrl === 'monitor-output-select') window.EveAudioflixGemini?.setMonitorSink?.(val && state().preferredSinkId === val ? '' : val, val && state().preferredSinkId === val ? 'Default monitor output' : lbl);
    ...
    function renderItemCard(item, type) {
        const isFrontend = (state().soundboardViewMode || 'backend') === 'frontend', volSlider = `<div class="audioflix-item-volume-wrapper" title="Volume"><input type="range" class="audioflix-volume-slider" min="0" max="1" step="0.01" value="${item.volume ?? 1}" data-af-type="${esc(type)}" data-af-id="${esc(item.id)}" style="--vol: ${(item.volume ?? 1) * 100}%"><span class="audioflix-volume-label">${Math.round((item.volume ?? 1) * 100)}%</span></div>`;
        const rep = activeRepeaters[item.id], repBadge = rep ? `<span class="audioflix-repeater-badge" style="background:#ea4335;color:#fff;padding:2px 6px;border-radius:3px;font-size:0.7rem;font-weight:bold;margin-right:4px;" title="Repeater active">🔁 Rep</span>` : '';
        ...
        return `<article class="audioflix-item-card"><div class="audioflix-playback-controls"><button type="button" class="audioflix-stop" data-af-action="stop-item" data-af-type="${esc(type)}" data-af-id="${esc(item.id)}" title="Stop">${stopSvg}</button><button type="button" class="audioflix-play" data-af-action="play" data-af-type="${esc(type)}" data-af-id="${esc(item.id)}" title="Play">${playSvg}</button></div><button type="button" class="audioflix-layer-play" data-af-action="layer-play" data-af-type="${esc(type)}" data-af-id="${esc(item.id)}" title="Layer Play">${layerPlaySvg}</button><div class="audioflix-item-body">${repBadge}<strong>${esc(item.title)}</strong><span>${esc(itemMeta(item))}</span>${type === 'sound' ? groupTags(item) : ''}</div><div class="audioflix-item-actions"><button type="button" class="audioflix-icon-btn" data-af-action="item-info" data-af-type="${esc(type)}" data-af-id="${esc(item.id)}">${cogSvg}</button>${delBtn}</div>${type === 'sound' ? volSlider : ''}</article>`;
    }

    function renderItems(items, type) {
        if (!items.length) return `<div class="audioflix-empty">No ${type === 'music' ? 'tracks' : 'sounds'} yet.</div>`;
        const isFrontend = (state().soundboardViewMode || 'backend') === 'frontend', filtered = type === 'sound' && isFrontend ? items.filter(isItemExposed) : items;
        if (!filtered.length) return `<div class="audioflix-empty">No exposed sounds yet. Switch to Backend view to expose sounds.</div>`;
        if (type === 'sound' && isFrontend) return renderFrontendActive();
        const groups = new Map(); filtered.forEach(it => { const k = groupKey(it, type); if (!groups.has(k)) groups.set(k, []); groups.get(k).push(it); });
        return [...groups.entries()].map(([n, gItems]) => {
            const cnt = gItems.length, coll = collapsedGroups[n] === true;
            return `<section class="audioflix-group ${coll ? 'is-collapsed' : ''}" data-af-group="${esc(n)}"><button type="button" class="audioflix-group-title" data-af-action="toggle-group" data-af-group="${esc(n)}" aria-expanded="${coll ? 'false' : 'true'}">${esc(n)}<span class="audioflix-group-count">${cnt} item${cnt === 1 ? '' : 's'}</span></button><div class="audioflix-item-grid">${gItems.map(it => renderItemCard(it, type)).join('')}</div></section>`;
        }).join('');
    }
    ...
    function renderForm(type) {
        const m = type === 'music';
        return `<form class="audioflix-form" data-af-form="${m ? 'music' : 'sound'}"><label><span>${m ? 'Track Title' : 'Sound Name'}</span><input name="title" required></label><label class="audioflix-wide-field"><span>URL / Path</span><input name="url" required></label><label><span>${m ? 'Artist' : 'Category'}</span><input name="${m ? 'artist' : 'category'}"></label><label><span>${m ? 'Folder' : 'Volume'}</span><input name="${m ? 'folder' : 'volume'}"></label><button type="submit" data-af-action="submit-form">${m ? 'Add Track' : 'Add Sound'}</button></form>`;
    }

    function renderPortsManager() {
        const ports = state().ports || [], list = ports.length ? ports.map(p => `<div class="audioflix-port-item"><div><strong>${esc(p.nickname)}</strong><code style="display: block; font-size: 0.8rem; color: #8ab4f8;">${esc(p.path)}</code></div><button type="button" class="audioflix-icon-btn danger" data-af-action="remove-port" data-af-id="${esc(p.id)}">${closeSvg}</button></div>`).join('') : '<div class="audioflix-empty">No ports configured.</div>';
    ...
    function renderAddSection(type) {
        ...
        const vBtn = `<button type="button" class="audioflix-view-toggle${isF ? ' is-active' : ''}" data-af-action="toggle-view-mode" style="margin-left: auto;">${isF ? 'Backend' : 'Frontend'}</button>`;
        const gBtn = `<button type="button" class="audioflix-add-toggle${groupsOpen ? ' is-active' : ''}" data-af-action="toggle-groups" style="margin-left: 8px;">Groups</button>`;
