...
(function (ns) {
    ...
    class AttractionEffect extends BaseEffect {
        ...
        drawFrame(time) {
            ...
            ctx.fillStyle = this.getThemeRgba('panelTint', 0.1);
            ...
            for (let i = 1; i < this.trail.length; i += 1) {
                ...
                ctx.strokeStyle = this.getThemeRgba('fxAccent', ratio * (0.12 + glow * 0.24));
            ...
            gradient.addColorStop(0, this.getThemeRgba('auraAccent', 0.44));
            gradient.addColorStop(0.35, this.getThemeRgba('mapAccent', 0.18));
            gradient.addColorStop(1, this.getThemeRgba('panelTint', 0));
    ...
    class LinesDotsEffect extends BaseEffect {
        ...
        drawFrame(time) {
            ...
            ctx.clearRect(0, 0, this.width, this.height);
            ctx.fillStyle = this.getThemeRgba('panelTint', 0.08);
            ctx.fillRect(0, 0, this.width, this.height);
            ...
            for (let i = 0; i < this.particles.length; i += 1) {
                ...
                for (let j = i + 1; j < this.particles.length; j += 1) {
                    ...
                    ctx.strokeStyle = this.getThemeRgba('fxAccent', alpha);
            ...
            this.particles.forEach((particle) => {
                ...
                const particleColorKey = i % 2 === 0 ? 'auraAccent' : 'mapAccent';
                halo.addColorStop(0, this.getThemeRgba(particleColorKey, 0.24 + this.glow * 0.15));
                halo.addColorStop(1, this.getThemeRgba(particleColorKey, 0));
                ctx.fillStyle = halo;
                ctx.beginPath();
                ctx.arc(particle.x, particle.y, 10 + this.glow * 8, 0, Math.PI * 2);
                ctx.fill();
                ctx.fillStyle = this.getThemeRgba(particleColorKey, 0.84);
                ctx.beginPath();
    ...
    class AuraCursorEffect extends BaseEffect {
        ...
        drawFrame(time) {
            ...
            coreGradient.addColorStop(0, this.getThemeRgba('auraAccent', 0.34));
            coreGradient.addColorStop(0.45, this.getThemeRgba('mapAccent', 0.12));
            coreGradient.addColorStop(1, this.getThemeRgba('panelTint', 0));
            ...
            this.orbiters.forEach((orbiter, index) => {
                ...
                ctx.strokeStyle = this.getThemeRgba('fxAccent', 0.08);
                ctx.lineWidth = 0.8;
                ctx.beginPath();
                ctx.moveTo(centerX, centerY);
                ctx.lineTo(x, y);
                ctx.stroke();
                ctx.fillStyle = this.getThemeRgba(index % 2 === 0 ? 'auraAccent' : 'mapAccent', 0.72);
                ctx.beginPath();
