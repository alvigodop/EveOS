...
(function (ns) {
    ...
    const renderInspectorCore = ns._renderInspectorCore || {};
    const markup = ns._renderInspectorRuntimeMarkup || {};
    const renderToolbarHelpers = ns._renderToolbarHelpers || {};
    const actionsApi = ns._coreActions || {};
    const { state, text, escapeHtml, getScopeText, getKindDisplayName, getStaticStateForNode, getNodePolarityState, getNodeCoverUrl, scheduleInspectorCoverRotation } = shared;
    const { getPrimaryAction, getSecondaryActions, buildInspectorHeader, buildCoverPanel, applyInspectorShellStyle } = renderInspectorCore;
    const { renderToolbarState } = renderToolbarHelpers;
    const { getCompactInspectorMarkup } = markup;
...
function updateCursor() {
    ...
    const moduleApi = ns._renderInspectorRuntimeView = ns._renderInspectorRuntimeView || {};
