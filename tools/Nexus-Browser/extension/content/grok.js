(() => {
  if (typeof window !== 'undefined' && globalThis.__browserAiBridgeGrokLoaded) return;
  if (typeof window !== 'undefined') globalThis.__browserAiBridgeGrokLoaded = true;

  const input = globalThis.BrowserAiBridgeGrokInput
    || (typeof module !== 'undefined' && module.exports ? require('./grok-input.js') : null);
  const answer = globalThis.BrowserAiBridgeGrokAnswer
    || (typeof module !== 'undefined' && module.exports ? require('./grok-answer.js') : null);

  if (!input || !answer) throw new Error('Grok bridge modules were not loaded in the expected order.');
  ...
  async function submitPrompt(requestId, text, { qualification = null } = {}) {
    ...
    if (!composer) {
      throw new Error('Grok composer was not found. Expected the Ask Grok anything editor or another visible contenteditable textbox.');
    }
    ...
    if (sendControl) {
      if (input.isUnsafeSendControl?.(sendControl)) {
        stopWatcher(requestId);
        throw new Error('Refusing to click a Grok microphone/voice control as the send button.');
      }
      sendControl.click();
      if (!exactOnce || await departed()) return;
      stopWatcher(requestId);
      throw new Error('Grok qualification prompt was not committed after the single allowed send-button click.');
    }

    dispatchComposerEnter(composer);
    if (exactOnce && !(await departed())) {
      stopWatcher(requestId);
      throw new Error('Grok qualification prompt was not committed after the single allowed Enter submission.');
    }
  }
  ...
  if (typeof chrome !== 'undefined' && chrome.runtime?.onMessage) {
    chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
      if (msg.type === 'bridge_ping') {
        sendResponse({ ok: true, adapter: 'grok' });
        return;
      }
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { ...input, ...answer, watchResponse, submitPrompt, stopWatcher, dispatchComposerEnter };
  }
})();
