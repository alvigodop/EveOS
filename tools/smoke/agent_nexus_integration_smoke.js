...
const DIAGNOSTIC_PATH = path.join(RESULT_DIR, 'agent-nexus-integration-failure.json');
...
function failureContext(output) {
    const lines = String(output || 'Unknown failure').replace(/\u001b\[[0-9;]*m/g, '').split(/\r?\n/)
...
if (failed) {
    ...
    fs.writeFileSync(DIAGNOSTIC_PATH, JSON.stringify({ timestamp: new Date().toISOString(), results }, null, 2));
    console.error(`AGENT_NEXUS_SMOKE: PASS ${results.length - 1} | FAIL 1`);
...
console.log(`AGENT_NEXUS_SMOKE: PASS ${results.length} | FAIL 0`);
