  ...
  try {
    ...
    const diagnostics = await page.evaluate(({ selector }) => {
      ...
      return {
        ...
        onDemand: node?.getAttribute('data-card-hydrate-on-demand') === '1',
        hydrating: node?.getAttribute('data-card-hydrating') === '1',
        cardText: String(node?.textContent || '').trim().slice(0, 800),
...
async function runSmoke(page) {
  ...
  const mainCardTitles = await waitForHydratedCardLinks(
    page,
    '.category-card[data-card-category="Beta"][data-card-workspace="main"]',
    'Main Alpha One',
    'Main renamed card'
  );
