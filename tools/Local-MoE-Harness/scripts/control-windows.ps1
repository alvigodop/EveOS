...
function Start-Harness {
    ...
    if ($ExistingId -and (Get-OwnedProcess $ExistingId @("run-harness-windows.ps1"))) { Write-Host "[Control] Harness process $ExistingId is already starting." }
        ...
        $Headless = ($env:EVEOS_HEADLESS -match '^(1|true|yes|on)$') -or ($env:LOCAL_MOE_HEADLESS -match '^(1|true|yes|on)$')
        if ($Headless) {
            ...
            $Process = Start-Process powershell.exe -ArgumentList $Args -WorkingDirectory $Root -PassThru
        }
